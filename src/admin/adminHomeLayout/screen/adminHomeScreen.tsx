...
import { mockUsersData, mockClientsData } from '../../../../mockdata';

// Import our subcomponents
import { SystemOverview, SystemAlert } from '../components/SystemOverview';
import { JobAnalytics, JobItem } from '../components/JobAnalytics';
import { EarningsDashboard, TransactionItem } from '../components/EarningsDashboard';
...
export function AdminHomeScreen({ session }: AdminHomeScreenProps) {
  ...
  const [alerts, setAlerts] = useState<SystemAlert[]>([
    {
      ...
      description: 'Nội dung công việc #TK-9021 có dấu hiệu vi phạm quy tắc cộng đồng.',
      targetId: '#TK-9021',
    },
    {
      id: 'alert_2',
      ...
      description: 'Lâm Nguyễn vừa gửi yêu cầu nâng cấp tài khoản lên Chuyên gia Dọn dẹp.',
      targetId: 'Lâm Nguyễn',
    },
  ]);
  ...
  const initialUsers: UserItem[] = [
    ...
    ...mockClientsData.map((c, idx) => ({
      ...
      status: 'active',
      avatar: c.avatar || `https://api.dicebear.com/7.x/identicon/svg?seed=${c.companyName}`,
    })),
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'jobs':
        return <JobAnalytics jobs={jobs} onViewJobDetail={(j) => handleViewTask(j.code)} />;
      case 'earnings':
        return (
          <EarningsDashboard
            balance={balance}
    ...
    <SafeAreaView className="flex-1 bg-[#F9FAFB]">
      ...
      <View className="px-5 pt-3 pb-3 bg-white border-b border-b-slate-100 flex-row justify-between items-center">
        <View className="flex-row items-center gap-3">
          ...
          <View>
            <Text className="text-slate-800 text-[18px] font-extrabold tracking-wide">Taskly Admin</Text>
            <Text className="text-purple-600 text-xs font-semibold">Quản trị viên: {displayName}</Text>
          </View>
        </View>
      ...
      {/* Sub-Header / Active Tab indicator with Hamburger Menu Button */}
      <View className="bg-white border-b border-b-slate-200/60 px-5 py-3 flex-row justify-between items-center">
        <View className="flex-row items-center gap-2">
          {activeTab === 'overview' && (
            <>
              <Home size={15} color="#7C3AED" />
              <Text className="text-[14px] font-extrabold text-slate-800">Tổng quan</Text>
            </>
          )}
          {activeTab === 'jobs' && (
            <>
              <Briefcase size={15} color="#7C3AED" />
              <Text className="text-[14px] font-extrabold text-slate-800">Công việc</Text>
            </>
          )}
          {activeTab === 'earnings' && (
            <>
              <DollarSign size={15} color="#7C3AED" />
              <Text className="text-[14px] font-extrabold text-slate-800">Thu nhập</Text>
            </>
          )}
          {activeTab === 'ai' && (
            <>
              <Brain size={15} color="#7C3AED" />
              <Text className="text-[14px] font-extrabold text-slate-800">Hiệu suất AI</Text>
            </>
          )}
          {activeTab === 'users' && (
            <>
              <UsersIcon size={15} color="#7C3AED" />
              <Text className="text-[14px] font-extrabold text-slate-800">Người dùng</Text>
            </>
          )}
        </View>
        ...
        <TouchableOpacity
          ...
          className="w-9 h-9 rounded-xl bg-slate-50 border border-slate-200 items-center justify-center active:bg-slate-100"
        >
          ...
          <TouchableWithoutFeedback>
            <View className="mx-5 bg-white rounded-2xl p-4 shadow-xl border border-slate-100">
              <View className="flex-row justify-between items-center mb-3 pb-2 border-b border-slate-100">
                <Text className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Danh mục quản trị</Text>
              ...
              <View className="gap-1">
                ...
                <TouchableOpacity
                  ...
                  className={`flex-row items-center gap-3 p-2.5 rounded-xl ${
                    activeTab === 'overview' ? 'bg-purple-50' : 'active:bg-slate-50'
                  }`}
                >
                  <View className={`w-8 h-8 rounded-lg items-center justify-center ${activeTab === 'overview' ? 'bg-purple-100' : 'bg-slate-100'}`}>
                    <Home size={14} color={activeTab === 'overview' ? '#7C3AED' : '#64748B'} />
                  </View>
                  <Text className={`text-[13px] font-bold ${activeTab === 'overview' ? 'text-purple-700' : 'text-slate-700'}`}>
                    Tổng quan hệ thống
                  </Text>
                </TouchableOpacity>
                ...
                >
                  <View className={`w-8 h-8 rounded-lg items-center justify-center ${activeTab === 'jobs' ? 'bg-purple-100' : 'bg-slate-100'}`}>
                    <Briefcase size={14} color={activeTab === 'jobs' ? '#7C3AED' : '#64748B'} />
                  </View>
                  <Text className={`text-[13px] font-bold ${activeTab === 'jobs' ? 'text-purple-700' : 'text-slate-700'}`}>
                    Quản lý công việc
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  className={`flex-row items-center gap-3 p-2.5 rounded-xl ${
                    activeTab === 'earnings' ? 'bg-purple-50' : 'active:bg-slate-50'
                  }`}
                >
                  <View className={`w-8 h-8 rounded-lg items-center justify-center ${activeTab === 'earnings' ? 'bg-purple-100' : 'bg-slate-100'}`}>
                    <DollarSign size={14} color={activeTab === 'earnings' ? '#7C3AED' : '#64748B'} />
                  </View>
                  <Text className={`text-[13px] font-bold ${activeTab === 'earnings' ? 'text-purple-700' : 'text-slate-700'}`}>
                    Thu nhập & Tài chính
                  </Text>
                </TouchableOpacity>
                ...
                >
                  <View className={`w-8 h-8 rounded-lg items-center justify-center ${activeTab === 'ai' ? 'bg-purple-100' : 'bg-slate-100'}`}>
                    <Brain size={14} color={activeTab === 'ai' ? '#7C3AED' : '#64748B'} />
                  </View>
                  <Text className={`text-[13px] font-bold ${activeTab === 'ai' ? 'text-purple-700' : 'text-slate-700'}`}>
                    Hiệu suất trợ lý AI
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  className={`flex-row items-center gap-3 p-2.5 rounded-xl ${
                    activeTab === 'users' ? 'bg-purple-50' : 'active:bg-slate-50'
                  }`}
                >
                  <View className={`w-8 h-8 rounded-lg items-center justify-center ${activeTab === 'users' ? 'bg-purple-100' : 'bg-slate-100'}`}>
                    <UsersIcon size={14} color={activeTab === 'users' ? '#7C3AED' : '#64748B'} />
                  </View>
                  <Text className={`text-[13px] font-bold ${activeTab === 'users' ? 'text-purple-700' : 'text-slate-700'}`}>
                    Quản lý người dùng
                  </Text>
                </TouchableOpacity>
              </View>
